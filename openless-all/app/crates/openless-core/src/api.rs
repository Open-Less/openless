...
impl OpenLessBackend {
    ...
    fn reserve_dictation_session(
        ...
        self.reserve_dictation_session_with_id(insert_text, SessionId::new(), None)
    }

    fn reserve_dictation_session_with_id(
        &self,
        insert_text: bool,
        session_id: SessionId,
        prefix_pcm: Option<Vec<u8>>,
    ...
    pub async fn resume_dictation_recording(
        &self,
        session_id: SessionId,
        pcm: Vec<u8>,
    ) -> Result<SessionId, BackendError> {
        if pcm.is_empty() || !pcm.len().is_multiple_of(2) {
            return Err(BackendError::new(
                BackendErrorCode::InvalidArgument,
                "recording recovery requires complete non-empty signed 16-bit PCM",
            ));
        }
        let reservation = self.reserve_dictation_session_with_id(true, session_id, Some(pcm))?;
        self.start_reserved_dictation(reservation, DictationStartOptions::default())
            .await
    }
    ...
    async fn start_reserved_dictation(
        ...
        context.recording.prefix_pcm = prefix_pcm;
        context.recording.elapsed_offset_ms = elapsed_offset_ms;
        let context = Arc::new(context);
        let context = {
            let mut state = self.state.write().expect("backend state lock poisoned");
            if let Err(error) = ensure_running(&state) {
    ...
    #[allow(clippy::too_many_arguments)]
    fn build_failed_dictation_session(
        &self,
        context: &DictationContext,
        session_id: SessionId,
        error_code: &str,
    ...
    fn cancel_session_adapters_preserving_archive(
        &self,
        session_id: SessionId,
    ) -> futures_util::future::BoxFuture<
        'static,
        Result<Option<Arc<dyn crate::ports::RecordingArchive>>, BackendError>,
    > {
        let engine = Arc::clone(&self.deps.dictation_engine);
        let resources = self.voice_sessions.hold_resources(session_id).ok();
        let insertion = self.cancel_text_insertion(session_id);
        ...
        own_voice_effect(
            ...
            Box::pin(async move {
                let _resources = resources;
                let archive = engine.cancel_preserving_archive(session_id).await?;
                insertion.await?;
                Ok(archive)
            }),
        )
    ...
    pub async fn cancel_dictation(
        ...
        self.cancel_dictation_impl(session_id, false)
            .await
            .map(|_| ())
    }

    pub async fn cancel_dictation_after_escape(
        &self,
        session_id: Option<SessionId>,
    ) -> Result<Option<crate::types::DictationRecordingRecovery>, BackendError> {
        self.cancel_dictation_impl(session_id, true).await
    }

    async fn cancel_dictation_impl(
        &self,
        session_id: Option<SessionId>,
        recover_after_escape: bool,
    ) -> Result<Option<crate::types::DictationRecordingRecovery>, BackendError> {
        let recovery_enabled =
            recover_after_escape && self.get_preferences().esc_recording_recovery_enabled;
        let (active, recovery_context) = {
            let mut state = self.state.write().expect("backend state lock poisoned");
            ensure_running(&state)?;
            ...
            let recovery_context = (recovery_enabled
                && matches!(
                    state.dictation.phase,
                    DictationPhase::Starting | DictationPhase::Recording
                ))
            .then(|| state.dictation_context.clone())
            .flatten();
            state.dictation.phase = DictationPhase::Cancelled;
            self.events.publish(
                Some(active),
        ...
        let pcm = match archive.read_pcm().await {
            Ok(pcm) if !pcm.is_empty() && pcm.len().is_multiple_of(2) => pcm,
            Ok(_) => return Ok(None),
            Err(error) => {
                log::warn!("failed to read cancelled recording for recovery: {error}");
                return Ok(None);
            }
        };
        ...
        if let Err(error) = self.upsert_history(
            ...
            log::warn!("failed to persist cancelled recording recovery: {error}");
            return Ok(None);
        }
        Ok(Some(crate::types::DictationRecordingRecovery {
            session_id: active,
            duration_ms,
        }))
    }

...
export const zhCN = {
  ...
  settings: {
    ...
    advanced: {
      ...
      localAsrDesc: '把转写工作从云端 ASR 改到本机推理。仅推荐离线 / 隐私敏感 / 不愿付费云 API 的场景。',
      localAsrWarningStrong: '本地推理跑在 CPU + Apple Silicon Accelerate 上，单次转写比云端 ASR 慢若干秒；中文识别准确率与方言/口音表现通常不如火山引擎 / Whisper turbo。启用前请确认你能接受这两点。',
      qwen3Desc: '阿里 Qwen3-ASR 本地权重，跨平台。模型文件首次使用需从 HuggingFace 下载。',
      foundryDesc: 'Microsoft Foundry Local 提供的 Whisper 模型，仅 Windows 可用。',
      enable: '启用',
      alreadyActive: '已启用',
      disableLocalLabel: '禁用本地 ASR',
      disableLocalDesc: '切回云端 ASR（默认火山引擎 bigasr）。需要在 Providers 里配置好相应的云端凭据。',
      disable: '禁用',
      platformNotSupported: '该平台暂未支持本地 ASR 模型集成。',
      confirmEnableLocalTitle: '确认启用本地 ASR',
      confirmEnableLocalBody: '启用「{{target}}」会把转写从云端切到本机推理。本地推理比云端慢若干秒、准确率通常更低。\n\n确认要启用吗？',
    },
    language: {
      title: '界面语言',

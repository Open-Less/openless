...
export const zhCN = {
  ...
  settings: {
    ...
    permissions: {
      ...
      iosKeyboard: {
        extensionLabel: 'OpenLess 键盘',
        enabled: '已启用',
        disabled: '未启用（设置 → 通用 → 键盘）',
        appGroupLabel: '共享容器 (App Group)',
        available: '可用',
        unavailable: '不可用',
        configLabel: '键盘转写配置',
        endpointPlaceholder: '转写端点（如 https://api.example.com/v1）',
        keyPlaceholder: 'API Key',
        modelPlaceholder: '模型（如 whisper-1）',
        promptPlaceholder: '转写提示词（可选）',
        save: '保存到键盘',
        saved: '已保存',
        hint: '配置写入 App Group 供键盘扩展读取；键盘需要开启「允许完全访问」才能联网与使用麦克风。',
      },

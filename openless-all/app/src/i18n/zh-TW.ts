...
export const zhTW: typeof zhCN = {
  ...
  settings: {
    ...
    advanced: {
      ...
      localAsrDesc: '把轉寫工作從雲端 ASR 改到本機推理。僅推薦離線 / 隱私敏感 / 不願付費雲 API 的場景。',
      localAsrWarningStrong: '本地推理跑在 CPU + Apple Silicon Accelerate 上，單次轉寫比雲端 ASR 慢若干秒；中文識別準確率與方言/口音表現通常不如火山引擎 / Whisper turbo。啓用前請確認你能接受這兩點。',
      qwen3Desc: '阿里 Qwen3-ASR 本地權重，跨平臺。模型文件首次使用需從 HuggingFace 下載。',
      foundryDesc: 'Microsoft Foundry Local 提供的 Whisper 模型，僅 Windows 可用。',
      enable: '啓用',
      alreadyActive: '已啓用',
      disableLocalLabel: '停用本地 ASR',
      disableLocalDesc: '切回雲端 ASR（默認火山引擎 bigasr）。需要在 Providers 裏配置好相應的雲端憑據。',
      disable: '停用',
      platformNotSupported: '該平臺暫未支持本地 ASR 模型集成。',
      confirmEnableLocalTitle: '確認啓用本地 ASR',
      confirmEnableLocalBody: '啓用「{{target}}」會把轉寫從雲端切到本機推理。本地推理比雲端慢若干秒、準確率通常更低。\n\n確認要啓用嗎？',
    },
    language: {
      title: '界面語言',

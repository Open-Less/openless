...
export const ja: typeof zhCN = {
  ...
  localAsr: {
    ...
    sherpaDesc: 'Windows では sherpa-onnx によるデバイス上の認識を使用します。ASR API キーは不要です。オフライン一括モデルは録音停止後に結果を返し、ストリーミングモデルは実験的で CPU 使用率と遅延の検証が必要です。',
    ...
    sherpaDiagnostics: '直近の診断',
    sherpaDiagPrepare: '読み込み {{value}}',
    sherpaDiagTranscribe: '推論 {{value}}',
    sherpaDiagAudio: '音声 {{value}}',
    sherpaLanguageJa: '日本語 ja',
    sherpaLanguageKo: '韓国語 ko',
    sherpaLanguageYue: '広東語 yue',
    sherpaModeOffline: '一括',
    sherpaModeOnline: 'ストリーミング',
    ...
    sherpaModelZipformerStreaming: 'Zipformer ストリーミング中英バイリンガル',
    sherpaModelZipformerStreamingDesc: '録音中に partial を返す実験的なオンラインモデル。CPU 使用率と遅延は Windows 実機での検証が必要です。',

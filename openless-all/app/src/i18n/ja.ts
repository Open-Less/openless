...
export const ja: typeof zhCN = {
  ...
  settings: {
    ...
    permissions: {
      ...
      iosKeyboard: {
        extensionLabel: 'OpenLess キーボード',
        enabled: '有効',
        disabled: '未有効（設定 → 一般 → キーボード）',
        appGroupLabel: '共有コンテナ（App Group）',
        available: '利用可能',
        unavailable: '利用できません',
        configLabel: '文字起こし設定',
        endpointPlaceholder: 'エンドポイント（例: https://api.example.com/v1）',
        keyPlaceholder: 'API キー',
        modelPlaceholder: 'モデル（例: whisper-1）',
        promptPlaceholder: '文字起こしプロンプト（任意）',
        save: 'キーボードに保存',
        saved: '保存しました',
        hint: '設定は App Group に書き込まれ、キーボード拡張から読み込まれます。ネットワークとマイクには「フルアクセス」が必要です。',
      },

...
export const ja: typeof zhCN = {
  ...
  settings: {
    ...
    advanced: {
      ...
      localAsrDesc: '転写処理をクラウド ASR から本機推論へ切り替えます。オフライン / プライバシー重視 / クラウド API 課金を避けたい場合のみ推奨。',
      localAsrWarningStrong: 'ローカル推論は CPU + Apple Silicon Accelerate で動作するため、1 回の転写時間はクラウド ASR より数秒長くなります。中国語や方言/訛りの認識精度も Volcengine / Whisper turbo に劣ることが一般的です。これらのトレードオフを理解した上で有効化してください。',
      qwen3Desc: 'Alibaba Qwen3-ASR の重みをローカルで実行（クロスプラットフォーム）。初回利用時に HuggingFace からモデルをダウンロードします。',
      foundryDesc: 'Microsoft Foundry Local の Whisper モデル。Windows のみで利用可能。',
      enable: '有効化',
      alreadyActive: '有効',
      disableLocalLabel: 'ローカル ASR を無効化',
      disableLocalDesc: 'クラウド ASR（既定は Volcengine bigasr）に戻します。「プロバイダー」でクラウド資格情報を設定しておく必要があります。',
      disable: '無効化',
      platformNotSupported: 'このプラットフォームではローカル ASR モデル統合に対応していません。',
      confirmEnableLocalTitle: 'ローカル ASR の有効化を確認',
      confirmEnableLocalBody: '「{{target}}」を有効化すると、転写処理がクラウドからローカル推論に切り替わります。ローカル推論は数秒遅く、認識精度も通常は劣ります。\n\n続行しますか？',
    },
    language: {
      title: '表示言語',

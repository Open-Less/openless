...
export const ko: typeof zhCN = {
  ...
  settings: {
    ...
    advanced: {
      ...
      localAsrDesc: '전사 작업을 클라우드 ASR 에서 본 기기 추론으로 전환합니다. 오프라인 / 프라이버시 중요 / 클라우드 API 비용 부담을 피하고 싶을 때만 권장됩니다.',
      localAsrWarningStrong: '로컬 추론은 CPU + Apple Silicon Accelerate 에서 동작하므로, 한 번의 전사 시간이 클라우드 ASR 보다 몇 초 더 걸립니다. 중국어 / 방언 / 억양 인식 정확도도 일반적으로 Volcengine / Whisper turbo 에 미치지 못합니다. 이러한 절충점을 받아들인 후 활성화하세요.',
      qwen3Desc: 'Alibaba Qwen3-ASR 가중치를 로컬에서 실행 (크로스 플랫폼). 처음 사용 시 HuggingFace 에서 모델 파일을 다운로드합니다.',
      foundryDesc: 'Microsoft Foundry Local 의 Whisper 모델. Windows 전용.',
      enable: '활성화',
      alreadyActive: '활성',
      disableLocalLabel: '로컬 ASR 비활성화',
      disableLocalDesc: '클라우드 ASR (기본 Volcengine bigasr) 로 돌아갑니다. "공급자" 에서 클라우드 자격 증명을 설정해 두어야 합니다.',
      disable: '비활성화',
      platformNotSupported: '이 플랫폼에서는 로컬 ASR 모델 통합이 아직 지원되지 않습니다.',
      confirmEnableLocalTitle: '로컬 ASR 활성화 확인',
      confirmEnableLocalBody: '"{{target}}" 을 활성화하면 전사가 클라우드에서 로컬 추론으로 전환됩니다. 로컬 추론은 몇 초 더 느리고 정확도도 보통 더 낮습니다.\n\n계속하시겠습니까?',
    },
    language: {
      title: '인터페이스 언어',

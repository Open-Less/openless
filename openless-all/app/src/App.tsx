...
const SelectionCorrectionBubble = lazy(() => import('./pages/SelectionCorrectionBubble').then(m => ({ default: m.SelectionCorrectionBubble })));
...
export function App({ isCapsule, isQa, isSelectionPolishPreview, isSelectionCorrection, isLessComputer, isLessComputerGlow, forcedOs }: AppProps) {
  if (isCapsule) {
  ...
  if (isSelectionCorrection) {
    return <Suspense fallback={null}><SelectionCorrectionBubble /></Suspense>;
  }
  if (isLessComputer) {

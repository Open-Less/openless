...
fn is_github_release_url(url: &str) -> bool {
    ...
    owner.eq_ignore_ascii_case("Open-Less")
        && repository == "openless"
        && releases == "releases"
        && download == "download"
        && !tag.is_empty()
...
impl fmt::Display for UpdateError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::Cancelled => f.write_str("更新已取消"),
...
/// Cancellation is accepted until the verified atomic replacement begins.
...
impl UpdateCancellation {
    ...
    fn begin_commit(&self) -> Result<(), UpdateError> {
        self.state
            .compare_exchange(
                0,
                2,
...
/// A fail-closed verifier for hosts which intentionally disable updates.
...
impl AppImageUpdater {
    ...
    pub async fn download_and_install(
        ...
        self.download_and_install_cancellable(manifest, progress, UpdateCancellation::default())
            .await
    }

    pub async fn download_and_install_cancellable(
        &self,
        manifest: UpdateManifest,
        ...
        let operation = async {
            ...
            let _cleanup = TemporaryDownload(download.clone());
            let result = async {
                ...
                install_verified_appimage_controlled(
                    &manifest,
                    self.expected_arch,
                    input,
                    &self.target,
        ...
        tokio::select! {
            biased;
            _ = cancellation.wake.notified() => Err(UpdateError::Cancelled),
...
fn install_verified_appimage_controlled(
    manifest: &UpdateManifest,
    expected_arch: &str,
...
fn commit_with_rollback(temp: &Path, target: &Path) -> Result<(), UpdateError> {
    commit_with_rollback_and_sync(temp, target, |parent| File::open(parent)?.sync_all())
}

fn commit_with_rollback_and_sync(
    temp: &Path,
    target: &Path,
    ...
    if let Err(error) = sync(parent) {
        fs::rename(&backup, target)
            .map_err(|error| io_error("restore AppImage after failed directory sync", error))?;
        sync(parent).map_err(|error| io_error("sync restored AppImage directory", error))?;
...
mod tests {
    ...
    #[test]
    fn cancellation_before_and_during_verification_never_replaces_the_image() {
        struct CancellingVerifier(UpdateCancellation);
        impl SignatureVerifier for CancellingVerifier {
            fn verify_base64_minisign(&self, _: &Path, _: &str) -> Result<(), UpdateError> {
                self.0.cancel();
                Ok(())
            }
        }
        for before in [true, false] {
            ...
            let result = install_verified_appimage_controlled(
                &manifest(b"new"),
                "x86_64",
                b"new".as_slice(),
                &target,
                &CancellingVerifier(cancellation.clone()),
                100,
                &cancellation,
            );
    ...
    #[test]
    fn cancellation_after_commit_starts_does_not_report_a_cancelled_install() {
        let cancellation = UpdateCancellation::default();
        cancellation.begin_commit().unwrap();
        assert!(!cancellation.cancel());
        cancellation.check().unwrap();
    }

    #[test]
    fn failed_directory_sync_restores_the_original_image() {
        let root = temp_dir("rollback");
        let target = root.join("OpenLess.AppImage");
        let temp = root.join("download");
        fs::write(&target, b"old").unwrap();
        fs::write(&temp, b"new").unwrap();
        let mut calls = 0;
        let result = commit_with_rollback_and_sync(&temp, &target, |_| {
            calls += 1;
            if calls == 1 {

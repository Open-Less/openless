...
pub fn vocab_page(
    ...
    vocab_card(
        ...
        |ui| {
            ui.horizontal_wrapped(|ui| {
                let preset_names = ["开发工具", "产品与平台", "技术术语", "英文写作"];
            ...
            if vm.vocab_editing_preset.is_some() {
                ...
                ui.horizontal(|ui| {
                        ...
                        .clicked()
                    {
                        ...
                        if !name.is_empty() {
                            actions.push(FrontendAction::VocabCreatePreset { name, phrases });
                        }
...
pub fn style_page(
    ...
    ui.horizontal(|ui| {
        ...
        ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
            ...
            if refresh.clicked() {
                vm.style_notice = Some("风格包列表已刷新".into());
            }
        });
    });
    ui.add_space(14.0);
                ...
                ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                    ...
                    let dictation = ui.add(
                        egui::Button::new(egui::RichText::new("语音润色").size(11.5).color(
                            if !vm.style_selection_workflow && !raw_active {
                                theme::SURFACE
                            } else {
                                theme::INK_3
                            },
        ...
        .show(ctx, |ui| {
            ...
            ui.horizontal(|ui| {
                ...
                if ui.button("恢复默认").clicked() {
                    vm.style_notice = Some("已恢复默认提示词（演示）".into());
                }

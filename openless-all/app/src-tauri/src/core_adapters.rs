...
impl RecordingArchive for TauriRecordingArchive {
    ...
    fn read_pcm(&self) -> BoxFuture<'static, Result<Vec<u8>, BackendError>> {
        ...
        Box::pin(async move {
            ...
            openless_core::decode_dictation_wav(&wav)
        })
    }
...
fn restore_or_discard_failed_start_archive(path: Option<&std::path::Path>, prefix: Option<&[u8]>) {
    let Some(path) = path else {
        return;
    };
    if let Some(prefix) = prefix {
        let result = openless_core::encode_dictation_wav(prefix).and_then(|wav| {
            std::fs::write(path, wav).map_err(|error| {
                BackendError::new(
                    BackendErrorCode::Persistence,
                    format!("restore cancelled recording archive: {error}"),
                )
            })
        });
        if let Err(error) = result {
            log::error!(
                "[recorder] failed to restore recovery archive after startup error: {error}"
            );
        }
    } else {
    ...
    fn start(
        ...
        Box::pin(async move {
            ...
            let (recording, runtime_errors) = tauri::async_runtime::spawn_blocking(move || {
                ...
                let elapsed_offset_ms = recording_plan.elapsed_offset_ms;
                let started_at = Instant::now();
                let level_progress = Arc::clone(&progress);
                let level_handler: Arc<dyn Fn(f32) + Send + Sync> = Arc::new(move |level| {
                    let elapsed_ms =
                        elapsed_offset_ms.saturating_add(started_at.elapsed().as_millis() as u64);
                    let _ = level_progress.publish_level(elapsed_ms, level);
                });
                ...
                let archive_prefix = recording_plan.prefix_pcm;
                let recovery_backup = archive_prefix.clone();
                let start_result = Recorder::start_with_archive_prefix(
                    microphone,
                    consumer,
                    level_handler,
                    recorder_archive_path,
                    archive_prefix,
                );
                let (recorder, runtime_errors, archive_active) = match start_result {
                    Ok(started) => started,
                    Err(error) => {
                        // Recorder::start may replace the WAV before native
                        // startup fails. Restore a recovery prefix; otherwise
                        // discard the new partial file.
                        restore_or_discard_failed_start_archive(
...
mod tests {
    ...
    #[test]
    fn failed_resume_start_restores_the_original_recording() {
        let path = std::env::temp_dir().join(format!(
            "openless-recovery-start-failure-{}.wav",
            uuid::Uuid::new_v4()
        ));
        std::fs::write(&path, b"partial").unwrap();
        let prefix = [1, 0, 2, 0];

        restore_or_discard_failed_start_archive(Some(&path), Some(&prefix));

        let wav = std::fs::read(&path).unwrap();
        assert_eq!(openless_core::decode_dictation_wav(&wav).unwrap(), prefix);
        restore_or_discard_failed_start_archive(Some(&path), None);
        assert!(!path.exists());
    }

      ...
      match message {
        WebViewMessage::CreateWebView(attrs) => {
          let Some((activity, web_chrome_client)) =
            ...
            // this used to be a debug_assertions-only eprintln!() (stderr,
            // never reached Android logcat) — replaced with a visible log
            // to confirm/deny whether CreateWebView is being silently
            // dropped here for a freshly rebuilt "main" window.
            log::warn!(
              "[wry-patch] CreateWebView dropped: no ACTIVITY_PROXY entry for activity_id={}",
              activity_id
            );
            return Ok(());
          };
          log::info!("[wry-patch] CreateWebView proceeding for activity_id={activity_id}");

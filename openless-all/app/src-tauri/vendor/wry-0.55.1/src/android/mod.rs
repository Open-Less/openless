...
pub unsafe fn android_setup(
  ...
  // OpenLess diagnostic patch (see src-tauri/vendor/wry-0.55.1).
  let had_webview_attributes = WEBVIEW_ATTRIBUTES.lock().unwrap().contains_key(&activity_id);
  log::info!(
    "[wry-patch] android_setup: activity_id={} had_webview_attributes={}",
    activity_id,
    had_webview_attributes
  );
...
impl InnerWebView {
  ...
  pub fn new(
    ...
    // OpenLess diagnostic patch (see src-tauri/vendor/wry-0.55.1): was
    // `.expect("no available activity")` — a panic here would unwind across
    // the JNI boundary (UB) instead of surfacing as a normal build() error,
    // and gave no visibility either way. Logged and turned into a proper
    // Err so a build() caller (e.g. ensure_main_webview_window()) actually
    // sees the failure instead of it vanishing silently or aborting.
    let activity_id = match activity_id_for_window_manager(window_manager) {
      Some(id) => {
        log::info!("[wry-patch] InnerWebView::new: resolved activity_id={id}");
        id
      }
      None => {
        log::warn!("[wry-patch] InnerWebView::new: no ACTIVITY_PROXY entry matches this window's WindowManager");
